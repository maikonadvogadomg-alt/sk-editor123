...
function MainPanel({
  ...
  const [view, setView] = useState<"home" | "push-new" | "push-existing" | "clone" | "pages-deploy">("home");
  ...
  const handlePagesDeploy = async () => {
    ...
    try {
      const res = await fetch("/api/pages/deploy", {
        ...
        body: JSON.stringify({ token: creds.token, username: creds.username, repoName: pagesRepo.trim() }),
      });
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (evt.type === "log") setPagesLogs(l => [...l, evt.msg]);
            else if (evt.type === "done") { setPagesUrl(evt.url); setPagesLogs(l => [...l, evt.msg]); }
            else if (evt.type === "error") { setPagesError(evt.msg); setPagesLogs(l => [...l, `❌ ${evt.msg}`]); }
        ...
        {!pagesRunning && !pagesUrl && (
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-3 py-2.5 bg-[#1c2714] border border-gray-700/50 rounded-xl text-sm text-gray-300 placeholder-gray-700 outline-none focus:border-blue-500/50"
            />
